...
def split_data(path, timesteps):
    ...
    X_train, y_train = create_sequences(train_data[['accel_x', 'accel_y', 'accel_z']],
...
def display_data(path):
    ...
    for activity in unique_activities:
        ...
        # Make a separate dataframe for each activity
        ...
        plt.xlabel('Time')
        plt.savefig(f'plots/scaled_data.png')
        plt.show()

        # plt.plot(subset['handAcc16gX'], color='red', label='AccX')
        # plt.plot(subset['handAcc16gY'], color='blue', label='AccY')
        # plt.plot(subset['handAcc16gZ'], color='green', label='AccZ')
        # plt.legend()
        # plt.show()
...
def extract_features(path, frequency, samples_required, train_features):
    ...
    X_train_sig, y_train = train_data[['accel_x', 'accel_y', 'accel_z']], train_data['activityId']
    X_test_sig, y_test = test_data[['accel_x', 'accel_y', 'accel_z']], test_data['activityId']
    ...
    if train_features:
        ...
        X_train.to_csv(f'features/X_train_acc_{frequency}.csv', index=False)
        X_test.to_csv(f'features/X_test_acc_{frequency}.csv', index=False)
    else:
        X_train = pd.read_csv(f'features/X_train_acc_{frequency}.csv')
        X_test = pd.read_csv(f'features/X_test_acc_{frequency}.csv')
...
def train_sequential_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model):
    ...
    file_name = f'models/acc_{chosen_model}_model.h5'
    ...
    #         model,
    #         to_file=f'models/{chosen_model}_structure.png',
    #         show_shapes=True,
    #         show_dtype=False,
...
def plot_confusion_matrix(y_test_labels, y_pred_labels, class_labels, chosen_model):
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'acc_{chosen_model}_cm_normalized.png'
        else:
            format = 'd'
