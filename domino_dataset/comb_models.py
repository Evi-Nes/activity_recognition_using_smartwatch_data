...
def preprocess_data(train_data, test_data, timesteps, unique_activities):
    ...
    """
    X_train, y_train = create_sequences(train_data[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']], train_data['activityId'],
...
def display_data(data, unique_activities):
    ...
    for activity in unique_activities:
        ...
        plt.xlabel('Time')
        plt.savefig(f'plots/comb_scaled_data.png')
        plt.show()
...
def train_sequential_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model, frequency):
    ...
    file_name = f'saved_models/comb_{chosen_model}_{frequency}_model.h5'
...
def extract_features(train_data, test_data, frequency, samples_required, train_features):
    ...
    """
    X_train_sig, y_train_sig = train_data[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']], train_data['activityId']
    X_test_sig, y_test_sig = test_data[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']], test_data['activityId']
    ...
    if train_features:
        ...
        X_train.to_csv(f'saved_features/X_train_comb_{frequency}.csv', index=False)
        X_test.to_csv(f'saved_features/X_test_comb_{frequency}.csv', index=False)
    else:
        X_train_features = pd.read_csv(f'saved_features/X_train_comb_{frequency}.csv')
        X_test_features = pd.read_csv(f'saved_features/X_test_comb_{frequency}.csv')
...
def train_feature_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model, frequency):
    ...
    if train_model:
        if chosen_model == 'rf':
            ...
            classifier.fit(X_train, y_train.ravel())
        file = open(f'models/comb_{chosen_model}_{frequency}_model.pkl', 'wb')
        pickle.dump(classifier, file)
    else:
        file = open(f'models/comb_{chosen_model}_{frequency}_model.pkl', 'rb')
