...
def split_data(path, timesteps):
    ...
    X_train, y_train = create_sequences(train_data[['gyro_x', 'gyro_y', 'gyro_z']],
...
def extract_features(path, frequency, samples_required, train_features):
    ...
    X_train_sig, y_train = train_data[['gyro_x', 'gyro_y', 'gyro_z']], train_data['activityId']
    X_test_sig, y_test = test_data[['gyro_x', 'gyro_y', 'gyro_z']], test_data['activityId']
    ...
    if train_features:
        ...
        X_train.to_csv(f'domino_dataset/features/X_train_gyro_{frequency}.csv', index=False)
        X_test.to_csv(f'domino_dataset/features/X_test_gyro_{frequency}.csv', index=False)
    else:
        X_train = pd.read_csv(f'domino_dataset/features/X_train_gyro_{frequency}.csv')
        X_test = pd.read_csv(f'domino_dataset/features/X_test_gyro_{frequency}.csv')
...
def train_sequential_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model):
    ...
    file_name = f'domino_dataset/models/gyro_{chosen_model}_model.h5'
...
def train_feature_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model):
    if train_model:
        if chosen_model == 'rf':
            ...
            classifier.fit(X_train, y_train.ravel())
        file = open(f'domino_dataset/models/gyro_{chosen_model}_model.pkl', 'wb') 
        pickle.dump(classifier, file)
    else:
        file = open(f'domino_dataset/models/gyro_{chosen_model}_model.pkl', 'rb') 
    ...
    print("Train Accuracy: %.2f%%" % (round(train_accuracy*100, 2)))
...
def plot_confusion_matrix(y_test_labels, y_pred_labels, class_labels, chosen_model):
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'gyro_{chosen_model}_cm_normalized.png'
        else:
            format = 'd'
