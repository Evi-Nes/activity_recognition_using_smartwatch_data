...
def train_sequential_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model, frequency):
    ...
    file_name = f'saved_models/gyro_{chosen_model}_{frequency}_model.h5'
...
def extract_features(train_data, test_data, frequency, samples_required, train_features):
    ...
    if train_features:
        ...
        X_train.to_csv(f'saved_features/X_train_gyro_{frequency}.csv', index=False)
        X_test.to_csv(f'saved_features/X_test_gyro_{frequency}.csv', index=False)
    else:
        X_train_features = pd.read_csv(f'saved_features/X_train_gyro_{frequency}.csv')
        X_test_features = pd.read_csv(f'saved_features/X_test_gyro_{frequency}.csv')
...
def train_feature_model(X_train, y_train, X_test, y_test, chosen_model, class_labels, train_model, frequency):
    ...
    if train_model:
        if chosen_model == 'rf':
            ...
            classifier.fit(X_train, y_train.ravel())
        file = open(f'models/gyro_{chosen_model}_{frequency}_model.pkl', 'wb')
        pickle.dump(classifier, file)
    else:
        file = open(f'models/gyro_{chosen_model}_{frequency}_model.pkl', 'rb')
...
def plot_confusion_matrix(y_test_labels, y_pred_labels, class_labels, chosen_model, frequency):
    ...
    for norm_value in normalize_cm:
        if norm_value == 'true':
            format = '.2f'
            plot_name = f'gyro_{chosen_model}_{frequency}_cm_norm.png'
        else:
            format = 'd'
            plot_name = f'gyro_{chosen_model}_{frequency}_cm.png'
